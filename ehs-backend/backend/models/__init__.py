...
class User(db.Model, UserMixin):
    ...
    role = db.Column(db.String(20), nullable=False) # 'patient', 'hospital', 'ambulance'
...
class Emergency(db.Model):
    ...
    volunteer_id = db.Column(db.String(50), nullable=True) # separate volunteer responder
...
class Pharmacy(db.Model):
    ...
    inventory = db.Column(db.Text, nullable=True) # JSON string of available critical medicines
...
class PharmacyInventory(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    pharmacy_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True) # can be null for global mock
    name = db.Column(db.String(100), nullable=False)
    category = db.Column(db.String(50), nullable=False)
    stock = db.Column(db.Integer, default=0)
    status = db.Column(db.String(20), default='OUT_OF_STOCK') # OPTIMAL, LOW, OUT_OF_STOCK
...
class ChatMessage(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    emergency_id = db.Column(db.String(50), db.ForeignKey('emergency.id'), nullable=False)
    sender_role = db.Column(db.String(20), nullable=False) # patient, ambulance, hospital
    message = db.Column(db.Text, nullable=False)
