    ...
    if expected_delivery_date:
        try:
            ...
            expected_delivery_date = datetime.strptime(
                expected_delivery_date.split("T")[0], "%Y-%m-%d"
            )
        except ValueError:
            expected_delivery_date = None

    risk_level = calculate_pre_registration_risk(
        medical_history, int(age) if age else 25
    )
...
def finish_consultation(consultation_id):
    ...
    # Enhanced Regex-based NLP processing logic
    transcript_lower = raw_transcript.lower()

    symptoms_map = {
        r"\b(fever|high temperature|hot)\b": "Pyrexia (Fever)",
        r"\b(pain|aching|sore)\b": "Pain reported",
        r"\b(cough|wheezing|shortness of breath|breathless)\b": "Respiratory Distress",
        r"\b(nausea|vomit|throwing up|sick to stomach)\b": "Nausea / Vomiting",
        r"\b(dizzy|faint|lightheaded)\b": "Dizziness / Syncope",
        r"\b(bleed|blood|cut|laceration)\b": "Hemorrhage / Bleeding",
        r"\b(bite|bitten|sting|stung)\b": "Animal / Insect Bite"
    }

    symptoms = set()
    for pattern, symptom in symptoms_map.items():
    ...
    if re.search(r"\b(chest)\b", transcript_lower) and "Pain reported" in symptoms:
        diagnosis = "Suspected Angina / Myocardial Infarction"
    elif re.search(r"\b(snake|venom|cobra|viper)\b", transcript_lower):
        diagnosis = "Snakebite Envenomation"
    elif re.search(r"\b(poison|pesticide|chemical)\b", transcript_lower):
        diagnosis = "Toxic Exposure / Poisoning"
    elif "Respiratory Distress" in symptoms and "Pyrexia (Fever)" in symptoms:
        diagnosis = "Suspected Severe Infection (Pneumonia/Flu/COVID)"
