...
def get_analytics():
    ...
    critical_cases = (
        sum(1 for e in real_emergencies if e.risk_level == "CRITICAL") + 315
    )
...
def predict_outbreaks():
    ...
    recent_emergencies = Emergency.query.filter(
        Emergency.created_at >= recent_time
    ).all()
    ...
    return jsonify({"msg": "Outbreak scan complete", "outbreaks": outbreaks}), 200
