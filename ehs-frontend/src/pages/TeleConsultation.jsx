...
const TeleConsultation = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col">
      {/* Header */}
      <div className="premium-glass-panel border-b border-white/10 p-4 sticky top-0 z-50 flex items-center justify-between">
      ...
      <div className="flex-1 p-4 md:p-6 flex flex-col lg:flex-row gap-6 relative">
        ...
        <div className="flex-1 relative flex flex-col">
          {isLoading && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/80 backdrop-blur-sm z-10 rounded-2xl">
              ...
              <p className="text-slate-300 font-medium">Connecting to secure video server...</p>
            </div>
          )}
          ...
          <motion.div 
            ...
            className="flex-1 bg-black rounded-2xl overflow-hidden border border-slate-800 shadow-2xl relative min-h-[400px]"
          >
          ...
          <div className="mt-4 p-4 rounded-xl bg-blue-900/20 border border-blue-500/30 flex items-start gap-3">
        ...
        <div className="w-full lg:w-80 flex flex-col gap-4">
          <motion.div 
            ...
            className="premium-glass-panel rounded-2xl p-5 border border-white/10"
          >
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 rounded-lg bg-white/5 border border-white/5">
                ...
                <div className="flex items-center gap-2">
                  <Heart size={16} className={`\${vitals.heart_rate > 100 ? 'text-red-500 animate-pulse' : 'text-green-400'}`} />
                  <span className="text-xl font-bold text-white">{vitals.heart_rate} <span className="text-xs text-slate-500">bpm</span></span>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center p-3 rounded-lg bg-white/5 border border-white/5">
                ...
                <div className="flex items-center gap-2">
                  <Activity size={16} className={`\${vitals.spo2 < 92 ? 'text-red-500' : 'text-blue-400'}`} />
                  <span className="text-xl font-bold text-white">{vitals.spo2} <span className="text-xs text-slate-500">%</span></span>
                </div>
              </div>
              
              <div className="flex justify-between items-center p-3 rounded-lg bg-white/5 border border-white/5">
                <span className="text-slate-400 text-sm">Blood Pressure</span>
                <span className="text-lg font-bold text-white">{vitals.blood_pressure}</span>
              </div>
            </div>
            
            <div className="mt-6 pt-4 border-t border-white/10">
              <p className="text-xs text-slate-400 text-center mb-2">Streaming to Neural Engine</p>
          ...
          <AnimatePresence>
            {mlInsight && mlInsight.is_anomaly && (
              <motion.div
                ...
                className="bg-red-950/80 border border-red-500/50 rounded-2xl p-5 shadow-[0_0_30px_rgba(239,68,68,0.2)]"
              >
                <div className="flex items-start gap-3">
                  <AlertTriangle className="text-red-500 shrink-0 mt-1 animate-pulse" size={24} />
                  <div>
                    <h3 className="text-red-400 font-bold text-lg leading-none mb-2">AI ANOMALY DETECTED</h3>
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-semibold text-red-300">Risk Score: {mlInsight.risk_score}/100</p>
                      {mlInsight.reasons && mlInsight.reasons.map((reason, idx) => (
                        <div key={idx} className="flex items-center gap-1.5 text-xs text-red-200">
          ...
          <motion.div 
            ...
            className="premium-glass-panel rounded-2xl p-4 border border-white/10 flex flex-col h-64 md:h-80"
          >
            <h2 className="text-sm font-bold text-white flex items-center gap-2 mb-3 border-b border-white/10 pb-2">
            ...
            <div className="flex-1 overflow-y-auto mb-3 space-y-2 pr-2 custom-scrollbar">
              {messages.length === 0 ? (
                ...
                messages.map((msg, idx) => (
                  <div key={idx} className={`text-sm p-2.5 rounded-xl max-w-[85%] \${msg.sender_role === (user?.role || 'patient') ? 'bg-blue-600 text-white ml-auto rounded-tr-none' : 'bg-slate-800 text-slate-200 mr-auto rounded-tl-none border border-slate-700'}`}>
                    <div className="text-[9px] uppercase tracking-wider opacity-70 mb-0.5 font-bold">
                      {msg.sender_role}
                    </div>
                    {msg.message}
                  </div>
            ...
            <form onSubmit={sendMessage} className="relative mt-auto">
              <input 
                ...
                className="w-full bg-slate-900 border border-slate-700 text-white text-sm rounded-xl py-2.5 pl-3 pr-10 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all shadow-inner placeholder:text-slate-500"
              />
              <button 
                type="submit" 
                disabled={!newMessage.trim()}
                className="absolute right-1.5 top-1.5 p-1.5 text-blue-400 hover:text-white hover:bg-blue-600 rounded-lg transition-colors disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:text-blue-400"
              >
                <Send size={16} />
              </button>
            </form>
